"""
Quick Backtest - 7 Days with Looser Parameters
Designed to generate trades for validation
"""
    ...
    for idx, row in data.iterrows():
        try:
            ...
            if result['signals']['enter_long'] and strategy.position is None:
                ...
                pnl -= abs(size * row['close']) * 0.0001  # Commission
                equity += pnl
                trades.append({
                    'pnl': pnl,
...
def main():
    """Run quick backtest with looser parameters."""
    print("="*80)
    print("VWAP CONTROL FLIP - QUICK BACKTEST (7 Days)")
    print("Using Looser Parameters to Generate Trades")
    print("="*80)
    
    symbols = ['GBPJPY']  # Start with one symbol
    
    # Very loose parameters - maximum trade generation for testing
    params_map = {
        'GBPJPY': StrategyParams(
            band_k=2.0,  # Standard band
            vol_mult=1.05,  # Very low volume requirement
            atr_cap_mult=4.0,  # Very high ATR cap (allows more volatility)
            require_nth_retest=1,  # 1st retest (most permissive)
            require_session_filter=False,  # No session filter
            cross_lookback_bars=20  # Longer lookback window
        )
    }
    ...
    for symbol, stats in results:
        print(f"{symbol:<10} {stats['trades']:<8} {stats['trades_per_day']:<12.2f} "
              f"{stats['win_rate']*100:<9.1f}% {stats['return']*100:<9.2f}% "
              f"{stats['sharpe']:<8.2f} {stats['max_dd']*100:<7.2f}%")
    
    print("\n" + "="*80)
    print("NOTE: Parameters are loosened for testing.")
    print("For production, use optimized parameters from QUBO tuner.")
    print("="*80)
