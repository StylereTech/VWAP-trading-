"""
Enhanced 60-Day Backtest with R:R Optimization
...
def backtest_enhanced(data: pd.DataFrame, symbol: str, params: EnhancedStrategyParams):
    ...
    if strategy.position is not None:
        ...
        equity += pnl
        trades.append({'pnl': pnl, 'rr': 0, 'side': pos.side})
...
def main():
    """Run enhanced 60-day backtest."""
    print("="*80)
    print("ENHANCED VWAP CONTROL FLIP - 60 DAY BACKTEST")
    print("With R:R Optimization & Stronger Filters")
    print("="*80)
    
    symbols = ['GBPJPY', 'BTCUSD', 'XAUUSD']
    
    # Enhanced parameters with symbol-specific tweaks
    params_map = {
        'GBPJPY': EnhancedStrategyParams(
            band_k=2.0,  # Tighter 2σ bands
            band_k_list=(1.0, 2.0, 3.0),
            vol_mult=1.5,  # Stronger volume requirement
            atr_cap_mult=2.0,  # Tighter ATR cap
        ...
        'BTCUSD': EnhancedStrategyParams(
            band_k=2.5,  # Wider bands for crypto
            band_k_list=(1.0, 2.5, 3.0),
            vol_mult=1.6,
            atr_cap_mult=2.0,
            require_nth_retest=3,
            ...
            require_session_filter=False,
            stretch_targets=True,
            min_rr_ratio=3.0,  # Higher R:R target for crypto
            breakeven_after_rr=1.0,
            require_trend_alignment=True,
            require_fvg_fill=True,
            atr_cap_tighter=2.0,
            require_volume_imbalance=True,
            volume_imbalance_threshold=1.3,
            symbol_type="CRYPTO"
        ),
        'XAUUSD': EnhancedStrategyParams(
            ...
            require_volume_imbalance=True,
            volume_imbalance_threshold=1.2,
            use_ema200_filter=True,  # Long-term filter for Gold
            symbol_type="METAL"
        )
    }
    ...
    for idx, symbol in enumerate(symbols):
        ...
        results = backtest_enhanced(data, symbol, params_map[symbol])
