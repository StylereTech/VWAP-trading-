"""
Generate Single Optimization Output JSON (Fast Version)
Reduced samples and days for faster execution
"""
...
def main():
    ...
    print("="*80)
    print("GENERATING OPTIMIZATION OUTPUT (FAST)")
    print("="*80)
    
    symbol = "GBPJPY"
    days = 14  # Reduced for speed
    samples = 50  # Reduced for speed
    seed = 7
    
    print(f"Symbol: {symbol}")
    print(f"Days: {days} (reduced for speed)")
    print(f"Samples: {samples} (reduced for speed)")
    print(f"Seed: {seed}")
    print("="*80)
    ...
    # Walk-forward validation (reduced test days)
    ...
    wf_result = walk_forward_validation(
        ...
        symbol=symbol,
        train_days=days,
        test_days=30  # Reduced for speed
    )
    ...
    xs, ys = [], []
    print("Generating samples for surrogate...")
